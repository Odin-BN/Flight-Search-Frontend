...
const DirectFlightCard: React.FC<Props> = ({flight}) => {
    ...
    <div className='flex justify-between items-center border-2 border-blue-500 rounded-lg p-4 shadow-md transition cursor-pointer bg-white card' onClick={handleClick}>
      ...
      <div className='flex-col flex justify-between items-center w-full text-center text-black'>
        ...
        <p className='m-0'>{flight?.infoPerItinerary?.[0].totalDuration|| "Tiempo de llegada desde que salio"}</p>
    ...
    <div className='flex justify-between items-center border-2 border-blue-500 rounded-lg p-4 shadow-md transition cursor-pointer bg-white card'>
        <div className='flex flex-col items-start text-left w-1/4'>
            ...
            <div className='flex gap-x-1 text-sm text-black w-full'>
                ...
                <p>-</p>
                <p>{flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary[1].flightSegments.length-1]?.arrivalDate || "fecha de llegada"}</p>
                <p>{flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary[1].flightSegments.length-1]?.arrivalTime || "hora de llegada"}</p>
            </div>
            ...
            <h3 className='flex gap-x-1 text-2xl font-bold'>
                ...
                <p>-</p>
                <p>{flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary[1].flightSegments.length-1]?.arrivalCity ||"aeropuerto llegada"}</p>
                <p>({flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary[1].flightSegments.length-1]?.arrivalAirport ||"aeropuerto llegada"})</p>
            </h3>

            <div className='flex gap-x-1 w-full text-sm text-black'>
                <p>{flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary.length-1]?.airlineName || "Nombre de aerolinea"}</p>
                <p>({flight?.infoPerItinerary?.[1]?.flightSegments?.[flight.infoPerItinerary.length-1]?.airlineCode || "Codigo Aerolinea"})</p>
            </div>
